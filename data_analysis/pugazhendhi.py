...
yahoo_fin[['Closing Price','Simple Moving Average','Upper Band', 'Lower Band']].plot(figsize=(16, 8)) # pylint: disable=line-too-long
...
yahoo_fin[['Closing Price', 'Simple Moving Average', 'UpperBand', 'LowerBand']].plot(figsize=(16, 8)) # pylint: disable=line-too-long
...
plt.plot(data["Closing Price"], label="EUR/INR Colsing Price", color='black')
plt.plot(data["Closing Price"].rolling(window=2).mean(), label="One Day Moving Averages", color='blue') # pylint: disable=line-too-long
buy = np.where((data['Closing Price'] > data["Closing Price"].rolling(window=2).mean()) & (data['Closing Price'].shift(1) < data["Closing Price"].rolling(window=2).mean().shift(1)), 1, np.nan) * 0.95 * data['Low'] # pylint: disable=line-too-long
sell = np.where((data["Closing Price"].rolling(window=2).mean() > data['Closing Price']) & (data["Closing Price"].rolling(window=2).mean().shift(1) < data['Closing Price'].shift(1)), 1, np.nan) * 0.95 * data['High'] # pylint: disable=line-too-long
...
plt.plot(data["Closing Price"], label="EUR/INR Colsing Price", color='black')
plt.plot(data["Closing Price"].rolling(window=7).mean(), label="One Week Moving Averages", color='blue')  # pylint: disable=line-too-long
buy = np.where((data['Closing Price'] > data["Closing Price"].rolling(window=7).mean()) & (data['Closing Price'].shift(1) < data["Closing Price"].rolling(window=7).mean().shift(1)), 1, np.nan) * 0.95 * data['Low']  # pylint: disable=line-too-long
sell = np.where((data["Closing Price"].rolling(window=7).mean() > data['Closing Price']) & (data["Closing Price"].rolling(window=7).mean().shift(1) < data['Closing Price'].shift(1)), 1, np.nan) * 0.95 * data['High']  # pylint: disable=line-too-long
plt.scatter(data.index, buy, color='green', marker='^', label='Buy Signal')
plt.scatter(data.index, sell, color='red', marker='v', label='Sell Signal')
